...
@section('content')
    <div class="container-fluid px-4">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('tickets.update', $id->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $id->title) }}">
                        @error('title')
                        ...
                        <div id="label-container">
                            @foreach (json_decode($id->labels, true) as $label)
                                <div class="input-group mb-2">
                                    <input type="text" class="form-control" name="labels[]" value="{{ $label }}" placeholder="Masukkan label">
                    ...
                    <div class="form-group mt-3">
                        ...
                        <div id="category-container">
                            @foreach (json_decode($id->categories, true) as $category)
                                <div class="input-group mb-2">
                                    <input type="text" class="form-control" name="categories[]" value="{{ $category }}" placeholder="Masukkan kategori">
                                    <div class="input-group-append">
                    ...
                    <div class="form-group mt-3">
                        ...
                        <select class="form-control @error('priority') is-invalid @enderror" id="priority" name="priority">
                            <option value="low" {{ old('priority', $id->priority) == 'low' ? 'selected' : '' }}>Low</option>
                            <option value="medium" {{ old('priority', $id->priority) == 'medium' ? 'selected' : '' }}>Medium</option>
                            <option value="high" {{ old('priority', $id->priority) == 'high' ? 'selected' : '' }}>High</option>
                        </select>
                        @error('priority')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
